...
export class VideoUpscaler {
    ...
    targetFPS;
    fps = 0;
    prevFrameRenderTime = -1;
    onFPSDrop;
    fpsRatio;
    destroyHandlers = [() => this.disable()];
    destroyed = false;
    resampleProgram;
    sharpenProgram;
    frameBuffer = null;
    ...
    videoTexture;
    constructor({ video, canvas, onFrameRendered = noop, targetFPS = DEFAULT_TARGET_FPS, onFPSDrop = noop, fpsRatio = DEFAULT_FPS_RATIO, }) {
        try {
            this.video = video;
            this.canvas = canvas;
    ...
    buildProgram(fragmentShaderSource) {
        const { gl } = this;
        ...
        gl.bufferData(gl.ARRAY_BUFFER, createRect({ width: 1, height: 1 }), gl.STATIC_DRAW);
        ...
        const setViewportSize = ({ width, height }) => {
            ...
            gl.bufferData(gl.ARRAY_BUFFER, createRect({ width, height }), gl.STATIC_DRAW);
        };
    ...
    getVideoFrameSize(frameMetadata) {
    ...
    renderFrame = (now, frameMetadata) => {
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, desiredFrameSize.width, desiredFrameSize.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
